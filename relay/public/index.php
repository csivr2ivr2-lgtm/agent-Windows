...
if ($method === 'POST' && $path === '/v1/audio/sessions') {
    ...
    $metadata = ['session_id'=>$id, 'codec'=>$codec, 'content_type'=>$type,
        'sample_rate'=>(int)($data['sample_rate'] ?? 0), 'channels'=>(int)($data['channels'] ?? 0), 'created_at'=>time()];
    file_put_contents($dir . DIRECTORY_SEPARATOR . 'metadata.json', json_encode($metadata), LOCK_EX);
    $received = [];
