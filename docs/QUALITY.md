...
After hardening, the same trace method measured **88.4% (1,169/1,323)** across Python production modules. CI installs `coverage.py`, measures line and branch coverage against `src/agent_windows` only, excludes tests/vendor/generated code, publishes XML, and enforces an honest 80% floor. CI is the authoritative coverage measurement.
