...
class ModelFitTests(unittest.TestCase):
    def test_quantization_and_estimation_validation(self):
        ...
        with self.assertRaises(ValueError): estimate_model_bytes(0, "q4", 8192)
        with self.assertRaises(ValueError): estimate_model_bytes(1, "q4", 0)

    def test_fit_verdicts_and_unknown_memory(self):
        small = assess_model_fit("small", 0.6, quantization="q4", context_tokens=4096, hardware=HARDWARE)
    ...
    def test_ollama_model_fit_and_report(self):
        tags = [
            ...
            {"name": "mystery", "details": {"parameter_size": "1.5B", "quantization_level": "Q8_0"}},
            {"name": "skip"},
        ]
        ...
        with mock.patch("agent_windows.model_fit.hardware_snapshot", return_value=HARDWARE), \
    ...
    def test_ollama_tags_network_and_json_fail_closed(self):
        ...
        with mock.patch("agent_windows.model_fit.urlopen", return_value=Response(payload)):
            self.assertEqual(model_fit._ollama_tags("http://localhost:11434/v1")[0]["name"], "x:1b")
        with mock.patch("agent_windows.model_fit.urlopen", side_effect=OSError("down")):
            self.assertEqual(model_fit._ollama_tags("http://localhost:11434/v1"), [])
        with mock.patch("agent_windows.model_fit.urlopen", return_value=Response(b"not-json")):
            self.assertEqual(model_fit._ollama_tags("http://localhost:11434"), [])

    def test_hardware_snapshot_uses_platform_memory_paths(self):
        with mock.patch("agent_windows.model_fit.sys_platform_is_windows", return_value=False), \
